...
| Narração | **TTS pt-BR feminino** infantil-amigável. Provider default **OpenAI `gpt-4o-mini-tts`** (Sprint 5b, 2026-05-17). Alternativa plugável: Gemini 2.5 Flash TTS. ElevenLabs descartada por decisão (sem key, sem plano de pegar) mas impl `core/tts/elevenlabs.py` permanece como fallback histórico. |
...
| Geração de vídeo | 🤖→👤 (2026-05-07, revisado 2026-05-17 pós-arena ELO): **Veo 3.1**, política de tier: `veo-3.1-lite-generate-preview` = **plumbing/dev** (default em `providers.yaml`, 1º E2E capado <$1); `veo-3.1-fast-generate-preview` = **teto de produção**; `veo-3.1-generate-preview` (Standard) = **morto** (Fast ≈ Standard dentro do erro por ~4× o preço). Status preview aceito. Kling 2.0, Runway, Seedance 2.0 continuam plugáveis (Seedance é galho §19). | **Sim, `VideoGenerator`** | Provider-agnostic é invariante. Veo entra como default pelo crédito; abstração intacta. |
...
| TTS | 👤 (sessão 2026-05-17, Sprint 5b): **OpenAI `gpt-4o-mini-tts`** como default; **Gemini 2.5 Flash TTS** como alternativa por config (`tts.provider`). ElevenLabs **descartada** (sem key, sem plano de pegar). Voz + instrução de estilo pt-BR em `config/voices.yaml`. A/B de produção decidido após pipeline verde (galho §19). | **Sim, `TTSProvider`** | Tanto OpenAI quanto Gemini usam keys já em mãos. Expressividade vocal é metade do produto; abstração permite trocar sem mexer no resto. |
...
| TTS (~75s narração) — **OpenAI `gpt-4o-mini-tts`** (default Sprint 5b) | ~US$0,015 (∼$0,60/1M caracteres) |
| TTS — Gemini 2.5 Flash TTS (alternativa) | ~US$0,02 |
| TTS — ElevenLabs (descartada, fallback histórico) | US$0,15–0,30 |
| Música | US$0 |
...
- **A/B de TTS pt-BR (OpenAI vs Gemini vs eventual Hume/clone).** Sprint 5b (2026-05-17) cravou OpenAI como default por ser key já em mãos + ter `instructions` que aceita orientação de estilo. Gemini 2.5 Flash TTS plugado em paralelo. ElevenLabs descartada por não-aquisição da key (sem plano de pegar). A/B decidido por evidência após pipeline verde — fora do escopo Sprint 5b. Reabrir quando: (a) voz OpenAI desafinar consistentemente em produção pt-BR, (b) Gemini melhorar a ponto de justificar swap, (c) caso de uso novo (clone Hume, custom voice) aparecer. Provider-agnostic já protege — só muda `config/providers.yaml::tts.provider`.
- **Subir tier do vídeo de Lite → Fast.** Sprint 5b setou `veo-3.1-lite-generate-preview` como default pra capar o 1º E2E em <$1. Lite é plumbing/dev; Fast é o **teto de produção** decidido em 2026-05-17 (arena ELO mostrou Fast ≈ Standard dentro do erro por ~4× menos). Subir: trocar `video.model` em `providers.yaml`. Critério: após o 1º E2E verde + amostra de clipes I2V com Clarinha mostrar qualidade aceitável. Standard fica **morto** salvo evidência empírica do contrário.
- **Seedance 2.0 como video provider.** Líder atual da arena ELO de geração de vídeo (2026-05-17). Não adotado por ora porque: (a) crédito Google está no Veo, (b) consistência da Clarinha precisa ser validada num provider antes de proliferar. Reabrir se a consistência da Clarinha decepcionar em clipes I2V reais do Veo Lite/Fast — Seedance entra como segunda hipótese. `VideoGenerator` abstrai.
